...
export default function ProductDetailsView({ product, avis = [], like = false }) {
    console.log(product)
    const gallery = [
        'uploads/temp/68dd1a5cdbdcd_1759320668.png',
        'uploads/temp/68dd1a5cdbdcd_1759320668.png',
    ]

    const categories = [
        "Offre exclusive", "Sauna", "Douche sensorielle", "Tisanerie"
    ]
        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-[#f9f7ed] px-8 py-4 col-span-2 lg:col-span-1 rounded-2xl">
                    <h4 className='text-4xl font-bold mb-4 text-[#333]'>Le Domaine des prés vertes & spa vinésime</h4>

                    <ProductCarousel gallery={product?.galleries_images} image={product?.image} />
                    <div className="flex flex-wrap gap-3 mt-10 font-tahoma">

                        {categories.map((cat) => (
                            <span className='bg-[#e2dfba] px-2 py-1 text-sm rounded'>{cat}</span>
                        ))}
                    </div>

                    <h4 className='text-4xl font-bold mb-4 text-[#333] my-2'>{product?.nom}</h4>
                    ...
                    <Link className='w-max rounded-md mx-auto mt-4 px-5 py-3 bg-black leading-4 text-white uppercase font-normal text-xs tracking-[3px] hover:bg-gray-800 transition font-tahoma flex items-center justify-center gap-2'><TranslatedText text="Voir l'établissement" /></Link>
                </div>

                <div className="bg-[#f9f7ed] px-8 py-4 col-span-2 lg:col-span-1 rounded-2xl">
                    {product?.exclusivite_spc && <img
                        loading="lazy"
                        src={exclusive}
                        alt="Exclusivité"
                    ...
                    <div className="border-b border-black my-4 font-tahoma">
                        <div className="py-3 flex items-center gap-2">
                            ...
                            <Link to={paths.recompense}><TranslatedText text="en savoir plus" className="underline" /></Link>
                        </div>
                    </div>
                    <LocationSection data={product?.spaData} />
                </div>
            </div>
