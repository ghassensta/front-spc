...
export default function ProchainementSection({ prochainement }) {
  ...
  return (
    <div className={`bg-[${backgroundColor}] w-screen relative bg-center left-[calc(-50vw+50%)] min-h-32 overflow-hidden py-4 px-3`}>
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row gap-2 md:gap-6">
          <div className="w-full md:w-1/2 py-8 px-3">
            <div className="flex gap-3">
              <span onClick={prevSlide} className="text-2xl border border-black rounded-full p-3 cursor-pointer">
                <FiChevronLeft />
              </span>
              <span onClick={nextSlide} className="text-2xl border border-black rounded-full p-3 cursor-pointer">
                <FiChevronRight />
              </span>
            </div>
            <h2 className="text-3xl">{prochainement?.title}</h2>
            <h3 className="text-[#B6B499] text-2xl">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <p className="font-tahoma text-sm">{cards[currentIndex].description}</p>
              </motion.div>
            </AnimatePresence>
          </div>
          <div className="w-full md:w-1/2 items-center justify-center">
            ...
            <div className="mt-5 mx-auto justify-center transform flex space-x-2">
              {cards.map((_, index) => (
                <button
                  key={index}
                  onClick={() => goToSlide(index)}
                  className={`w-3 h-3 rounded-full ${currentIndex === index ? 'bg-[#B6B498]' : 'bg-gray-400'} hover:bg-[#B6B498] transition-colors duration-300`}
