...
export default function CategoryPuce({ slug = "#", icon = null, name = "" }) {
  return (
    ...
    >
      <Link
        to={`/categories/${slug}`}
        className="group w-full inline-block rounded-full min-w-24 bg-[#B6B499] pr-6 pl-1 hover:bg-black duration-300 "
      >
        <div className="flex items-center gap-1 text-center text-white relative">
          ...
          <div className="relative">
            <span className="text-white text-sm text-left md:text-center font-tahoma uppercase">{name}</span>
            {/* Underline animation */}
            <motion.span
